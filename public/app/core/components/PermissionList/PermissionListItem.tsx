...
function ItemDescription({ item }: { item: DashboardAcl }) {
  if (item.userId) {
    return <span className="filter-table__weak-italic">(Utilisateur)</span>;
  }
  if (item.teamId) {
    return <span className="filter-table__weak-italic">(Equipe)</span>;
  }
  return <span className="filter-table__weak-italic">(Rôle)</span>;
}
    ...
    return (
      <tr className={setClassNameHelper(Boolean(item?.inherited))}>
        ...
        <td>
          ...
          {inheritedFromRoot && <em className="muted no-wrap">Permission par défaut</em>}
        </td>
        <td className="query-keyword">peux</td>
        <td>
          <Select
            isSearchable={false}
